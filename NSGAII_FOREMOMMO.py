...
class NSGAII_FOREMOMMO:
    def __init__(self, objectionFunction,limitList, precisionList, population, generation, needArchive = False):
        self._objectionFunction = objectionFunction
        self._limitList = limitList
        self._precisionList = precisionList
    ...
    def _Dgrid(self, group, t):
        ...
        for i in range(N):
            tmpX = []
            for j in range(dimention):
                tmp = int((N-1) * (x[i][j] - minList[j]) / (maxList[j] - minList[j])) + 1
                tmpX.append(tmp)
            tx.append(tmpX)
        ...
        for i in range(N):
            ...
            for j in range(N):
                if(i==j):
                    ...
                    dMatrix[i][j] = distance(tx[i], tx[j])
                if(dMatrix[i][j] < min):
                    min = dMatrix[i][j]
            if(min > sigma):
                sigma = min
    ...
    def _getObjectionFunctionsValues(self, group, t):
        valueList = []
        for i in range(len(group)):
            valueList.append(self._objectionFunction(group[i].Decode()))
        DgridList = self._Dgrid(group, t)
        res = []
        for i in range(len(group)):
            tmp = []
            tmp.append(valueList[i])
            tmp.append(DgridList[i])
            res.append(tmp)
        return res
    ...
    def _InitGroup(self): # 初始化种群
        ...
        values = self._getObjectionFunctionsValues(self._lastGroup, 1)
        for i in range(len(values)):
            self._lastGroup[i].SetFitness(values[i])
    ...
    def _FastNondominatedSort(self, group, justFitst = False):
        ...
        for i in range(size):
            for j in range(size):
                if(i == j):
                    continue
                if(self._Domination_big(group[i].GetFitNess(), group[j].GetFitNess())):
                    group[i].AddDomination(group[j])
                elif(self._Domination_big(group[j].GetFitNess(), group[i].GetFitNess())):
                    group[i].AddNp()
                    if(justFitst):
                        break
            if(group[i].GetNp() == 0):
    ...
    def _Copy(self, t): # 拷贝  先计算适应度，然后进行非支配排序，然后进行抽样拷贝
        #print("开始拷贝")
        res = self._getObjectionFunctionsValues(self._group, t)
        for i in range(len(res)):
            self._group[i].SetFitness(res[i])
    ...
    def _Variation(self): # 变异
        ...
        for _ in range(variationCount):
            index = int((int(random.random()) * MAX) % totalCount)
...
if __name__ == "__main__":
    nsga = NSGAII_FOREMOMMO(funcA, [[-10,10]], [10], 200, 30, True)
    g, a = nsga.Fit()
    print(g)